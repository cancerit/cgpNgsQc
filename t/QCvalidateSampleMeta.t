...
subtest 'validate_bam'  => sub {
  local $SIG{__WARN__} = sub {};
  my $got;
  $got = Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_bam('test', 'random', 'file should not exist please', 'GRCh37d5', 100);
  is($got,'failed:invalid is_normal value; bam is not found', 'bad is_normal and if input exists');
  $got = Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_bam('test', 'y', $test_sample_1_file, 'GRCh37d5', 100);
...
subtest 'validate_samples'  => sub {
  ...
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0,count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: can not find input/, 'invalid input file';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'invalid input file, expecting to die';

  $test_input = File::Spec->catfile($test_data,'test-meta_no_sampleIDcolumn.xlsx');
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: Sample_ID column is missing!/, 'no sample_id header';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'no sample_id header, expecting to die';

  $test_input = File::Spec->catfile($test_data,'test-meta_dup_donorIDcolumn.xlsx');
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: Duplicated header Donor_ID found!/, 'dup donor_id header';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'dup donor_id header, expecting to die';

  $test_input = File::Spec->catfile($test_data,'test-meta_no_header_1columnShort.xlsx');
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: Require 5 columns, 4 found!/, 'no header, a column short';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'no header, a column short, expecting to die';

  $test_input = File::Spec->catfile($test_data,'test-meta_dup_sampleID.xlsx');
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: Duplicated Sample_ID found for /, 'dup sample id';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'dup sample id, expecting to die';

  $test_input = File::Spec->catfile($test_data,'test-meta_dup_bamPath.xlsx');
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: Duplicated relative_file_path found: /, 'dup bam file';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'dup bam file, expecting to die';

  $test_input = File::Spec->catfile($test_data,'test-meta_invalid_bam.xlsx');
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/$test_xlsx_file/, 'not a real bam';
  dies_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } 'not a real bam, expecting to die';
};
