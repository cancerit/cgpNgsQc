...
subtest 'Initialisation checks' => sub {
  ...
  use_ok($MODULE);
  new_ok($MODULE => [{ i => $test_xlsx_file, o => $tmp, f => 'xlsx'}]);
};
...
subtest 'validate_samples'  => sub {
  ...
  like($got[3], qr/$expect[3]$/, 'input with header, good input, output line, raw bam 2');
  ...
  throws_ok { Sanger::CGP::NgsQc::ValidateSampleMetaAndBamHeader::validate_samples({i => $test_input, f => 'xlsx', mod => 0, count_base_number => 100, genome_build => 'GRCh37d5'}) } qr/Error: Require 6 columns, 5 found!/, 'no header, a column short';
